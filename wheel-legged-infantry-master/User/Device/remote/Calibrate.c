/**
  ...
  *             unsigned char cali_xxx_hook(uint32_t *cali, unsigned char cmd), and add the name in "cali_name[CALI_LIST_LENGHT][3]"
  ...
  *             unsigned char cali_xxx_hook(uint32_t *cali, unsigned char cmd), 添加新名字在 "cali_name[CALI_LIST_LENGHT][3]"
...
static void RC_cmd_to_calibrate(void)
{
    ...
    else if (rc_action_flag == 3 && rc_cmd_time > RC_CMD_LONG_TIME)
    {
        ...
        head_cali.temperature = (int8_t)(35) + 10;

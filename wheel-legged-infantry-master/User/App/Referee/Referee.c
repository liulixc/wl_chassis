...
void ui_draw_pitch_change_init()
{
    ext_graphic_one_data_t ui_pitch;
    //裁判通信帧头
    ui_pitch.txFrameHeader.SOF = REFREE_HEADER_SOF;
    ui_pitch.txFrameHeader.data_length = sizeof (ext_student_interactive_header_data_t) +
                                         sizeof (ui_graphic_data_struct_t) * 1;
    ui_pitch.txFrameHeader.seq = 0;//包序号设置为0
    memcpy(ClientTxBuffer, &ui_pitch.txFrameHeader, sizeof (frame_header_struct_t));//把帧头放进去
    //CRC8校验帧头
    append_CRC8_check_sum(ClientTxBuffer, sizeof (frame_header_struct_t));
    ui_pitch.CmdID = Referee_ID_robot_interactive_header_data;
    //数据帧头
    ui_pitch.dataFrameHeader.send_ID = Referee.GameRobotStat.robot_id;
    ui_pitch.dataFrameHeader.receiver_ID = Referee.SelfClient;
    ui_pitch.dataFrameHeader.data_cmd_id = UI_INTERACT_ID_draw_one_graphic;
    //数据内容填充

    //中间橙色刻度线
    Figure_Graphic(&ui_pitch.clientData, "LIO", UI_ADD, UI_ARC, UI_ZERO_LAYER, UI_CYAN_BLUE,
                   89, 91, 45, 960, 539, 0, 360, 360);
    //把除去帧头的其他部分放进缓存区
    memcpy(ClientTxBuffer + Referee_LEN_FRAME_HEAD, (uint8_t*)&ui_pitch.CmdID, sizeof (ui_pitch));
    //帧尾使用CRC16处理
    append_CRC16_check_sum(ClientTxBuffer, sizeof (ui_pitch));
    //串口6发送
    usart6_tx_dma_enable(ClientTxBuffer, sizeof (ui_pitch));
    osDelay(100);
}
...
void dynamic_cap_percentage_draw()
{
    ...
    ui_cap_percentage.dataFrameHeader.data_cmd_id=UI_INTERACT_ID_draw_two_graphic;
    //数据填充
    uint32_t real_cap = cap_percentage * 216;
...
{
    if( gimbal_msg.gimbalCtrlMode == GIMBAL_AUTO )//自瞄模式，ui变紫红色
    {
        uiColor.auto_aim_color = UI_FUCHSIA;
    }
    else if(gimbal_msg.gimbalCtrlMode == GIMBAL_BUFF )//大符
    {
        uiColor.auto_aim_color = UI_RED_BLUE;//红蓝
    }
    else if(gimbal_msg.gimbalCtrlMode == GIMBAL_SBUFF)//小符
    {
    ...
    if(chassis.chassis_ctrl_mode == CHASSIS_SPIN)//小陀螺模式，ui变紫红色
    {
...
//    }
    ...
    if(ABS(gimbal_msg.launcherCtrlInfo.fire_l_speed_rpm) > 500 && ABS(gimbal_msg.launcherCtrlInfo.fire_l_speed_rpm) > 500)
    {
        uiColor.fire_color = UI_FUCHSIA;//摩擦轮启动后呈现紫红色
    }
    else
...
void ui_aim_draw()
{
    ...
    //数据内容填充
    //竖线
    Figure_Graphic(&ui_aim.clientData[0], "LI0", UI_ADD, UI_LINE, UI_ZERO_LAYER, UI_YELLOW,
                   0, 0, 2, 669, 1, 0, 838, 283);
    //横线1
    Figure_Graphic(&ui_aim.clientData[1], "LI1", UI_ADD, UI_LINE, UI_ZERO_LAYER, UI_YELLOW,
                   0, 0, 2, 1141, 283, 0, 1327, 1);
    //横线2
    Figure_Graphic(&ui_aim.clientData[2], "LI2", UI_ADD, UI_LINE, UI_ZERO_LAYER, UI_YELLOW,
                   0, 0, 2, 932, 376, 0, 992, 376);
    //横线3
    Figure_Graphic(&ui_aim.clientData[3], "LI3", UI_ADD, UI_LINE, UI_ZERO_LAYER, UI_ORANGE,
                   0, 0, 3, 945, 353, 0, 976, 353);
    //圆型
    Figure_Graphic(&ui_aim.clientData[4], "LI4", UI_ADD,  UI_CIRCLE, UI_ZERO_LAYER, UI_GREEN,
...
void dynamic_pitch_draw()
{
    ext_graphic_one_data_t ui_pitch_change;
    //裁判通信帧头
    ui_pitch_change.txFrameHeader.SOF = REFREE_HEADER_SOF;
    ui_pitch_change.txFrameHeader.data_length = sizeof (ext_student_interactive_header_data_t) +
                                                sizeof (ui_graphic_data_struct_t) * 1;
    ui_pitch_change.txFrameHeader.seq = 0;//包序号设置为0
    memcpy(ClientTxBuffer, &ui_pitch_change.txFrameHeader, sizeof (frame_header_struct_t));//把帧头放进去
    //CRC8校验帧头
    append_CRC8_check_sum(ClientTxBuffer, sizeof (frame_header_struct_t));
    ui_pitch_change.CmdID = Referee_ID_robot_interactive_header_data;
    //数据帧头
    ui_pitch_change.dataFrameHeader.send_ID = Referee.GameRobotStat.robot_id;
    ui_pitch_change.dataFrameHeader.receiver_ID = Referee.SelfClient;
    ui_pitch_change.dataFrameHeader.data_cmd_id = UI_INTERACT_ID_draw_one_graphic;
    //数据内容填充

    //橙色刻度线,每调用这个函数，我暂时注释掉ty
//    Figure_Graphic(&ui_pitch_change.clientData, "LIO", UI_MODIFY, UI_ARC, UI_ZERO_LAYER, UI_ORANGE,
//                   (unsigned long)gimbal_msg.pitch.relative_angle_get+32-1, (unsigned long)gimbal_msg.pitch.relative_angle_get+32+1, 45, 960, 539, 0, 360, 360);

    //把除去帧头的其他部分放进缓存区
    memcpy(ClientTxBuffer + Referee_LEN_FRAME_HEAD, (uint8_t*)&ui_pitch_change.CmdID, sizeof (ui_pitch_change));
    //帧尾使用CRC16处理
    append_CRC16_check_sum(ClientTxBuffer, sizeof (ui_pitch_change));
    //串口6发送
    usart6_tx_dma_enable(ClientTxBuffer, sizeof (ui_pitch_change));
    osDelay(100);
}
...
_Noreturn void UI_paint_task(void const*argument)
{
    ...
    while(1)
    {
        if(KeyBoard.V.click_flag == 0)
        {
            ...
            //ui_line5_draw();//画pitch刻度线

            //ui_draw_pitch_change_init();//pitch角度显示
